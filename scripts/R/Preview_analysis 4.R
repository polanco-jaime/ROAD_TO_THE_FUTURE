# Bacon Demposition 
# 10p
data_10p$year_treated = data_10p$year_treated_10p
table(is.na(data_10p$year_treated) )
data_10p = balancing_data(data= data_10p, id = 'cole_cod_dane_institucion', time_references = 'year' )

data_10p$sd_math_i
data_10p_filtered <- data_10p[,c('sd_math_i','sd_reading_i','cole_cod_dane_institucion', 'year', 'year_treated_10p', 'ESTU_CONSECUTIVO') ] %>%
  group_by(cole_cod_dane_institucion) %>%
  filter(all(unique(year) %in% year))

df_bacon <- bacon(sd_math_i~time_to_treat_10p,
                  data = data_10p,
                  id_var = "cole_cod_dane_institucion",
                  time_var = "year" )
summary(data_10p_filtered)
...
library(data.table)
library(ggplot2)
data$year_treated_ai
data = data_ai[ , c(  "sd_math_i"   , "treat_" ,  "year_treated"  ,  "year" ,"cole_cod_dane", "estu_trabaja",  "cole_naturaleza"   , "buffer_km"  )]
# data = na.omit(data)
